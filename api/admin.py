...
from .models import RateLimitAttempt, RateLimitBlock, RateLimitRule
...
class RecentRateLimitAttemptInline(admin.TabularInline):
    ...
    def count_colored(self, obj):
        if obj.count >= obj.rule.limit:
            return format_html('<span style="color:#dc3545;font-weight:bold">{}</span>', obj.count)
        elif obj.count >= obj.rule.limit * 0.75:
            return format_html('<span style="color:#fd7e14">{}</span>', obj.count)
        return obj.count
...
class ActiveRateLimitBlockInline(admin.TabularInline):
    ...
    def status_colored(self, obj):
        ...
        if obj.blocked_until > now:
            return format_html('<span style="color:#dc3545;font-weight:600">BLOCKED</span>')
...
class RateLimitRuleAdmin(admin.ModelAdmin):
    ...
    def endpoint_pattern_short(self, obj):
        if not obj.endpoint_pattern:
            return "—"
        return (
            obj.endpoint_pattern[:38] + "…"
            if len(obj.endpoint_pattern) > 38
            else obj.endpoint_pattern
        )
    ...
    def attempt_count_last_24h(self, obj):
        ...
        count = RateLimitAttempt.objects.filter(rule=obj, last_attempt__gte=since).count()
        if count == 0:
            return "—"
        elif count < 10:
            return str(count)
...
class RateLimitAttemptAdmin(admin.ModelAdmin):
    ...
    def count_colored(self, obj):
        if obj.count >= obj.rule.limit:
            return format_html(
                '<span style="color: #dc3545; font-weight: bold;">{}</span>', obj.count
            )
