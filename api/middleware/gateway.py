...
class GatewayControlMiddleware:
    ...
    def _set_request_metadata(request):
        ...
        request.is_authenticated = bool(isinstance(request.user, User))
    ...
    def _validate_api_client(self, request):
        ...
        if system.allowed_ips and request.client_ip not in system.allowed_ips:
    ...
    def _check_rate_limit(self, request) -> dict:
        ...
        api_client_id = (
            str(request.api_client.id)
            if getattr(request, "api_client", None)
            else f"anon-{client_ip}"
        )
        ...
        for rule in rules:
            ...
            limit_key = self._make_limit_key(
                rule.scope, api_client_id, user_id, client_ip, endpoint
            )
